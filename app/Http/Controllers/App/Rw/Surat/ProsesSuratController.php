...
namespace App\Http\Controllers\App\Rw\Surat;
...
class ProsesSuratController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $data['compact'] = $data;
        return view('app.rw.surat.proses', $data);
    }
    ...
    public function simpan(Request $request)
    {
        try {
            ...
            $SURAT_STATUS = $request->disetujui == 1 ? $status[3] : $status[0];
            ...
            if ($request->disetujui == 1) {
                // ambil di pengaturan
                $pegawai_penerima_surat_id = settings()->get('setting.penerimma_surat.pegawai_id');
                $pegawai = Pegawai::find($pegawai_penerima_surat_id);
                if (is_null($pegawai)) {
                    return response()->json([
                        'message' => 'Pengawai penerima dari pihak desa belum di atur, silahkan hubungi pihak desa. Terima Kasih',
                    ], 500);
                }

                // jika disetujui
                $tracking_surat->ke_nama = $pegawai->penduduk->nama;
                $tracking_surat->ke_nip = $pegawai->nip;
            } else {
                // jika ditolak
