...
class NikahController extends Controller
{
    ...
    public function perbaiki(Surat $surat)
    {
        ...
        $nik_ayah_ada = Penduduk::where('nik', $surat->nikah->ayah_nik)->count() > 0;
        $nik_ibu_ada = Penduduk::where('nik', $surat->nikah->ibu_nik)->count() > 0;
        $nik_anak_ada = Penduduk::where('nik', $surat->nikah->anak_nik)->count() > 0;
        $nik_calon_ada = Penduduk::where('nik', $surat->nikah->calon_nik)->count() > 0;
