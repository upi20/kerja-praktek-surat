...
class TrackingController extends Controller
{
    ...
    public function batalkan(Request $request)
    {
        try {
            ...
            $SURAT_DI_PENDUDUK = config('app.status_surats')[0];
            ...
            if (!in_array($surat->status, [$SURAT_DI_PENDUDUK,  $SURAT_DI_RT, $SURAT_DI_RW])) {
                return ResponseFormatter::error(
                    [
                        'surat' => $surat,
                        'tracking' => $tracking
                    ],
                    'Surat hanya bisa dibatalkan ketika berada di pihak Penduduk, Rukun Tetangga dan Rukun Warga',
                    403
                );
            }
